  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-1">
        ...
        >
          <Clock className="w-2 h-2 sm:w-3 sm:h-3 mr-1" />6 Active
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        {benefitsData.map((benefit, index) => (
          ...
          >
            <Card
              className={`h-full transition-all duration-500 relative overflow-hidden cursor-pointer ${getCardStyles(theme)}`}
              style={{
                boxShadow:
              ...
              <CardHeader className="pb-2 sm:pb-3 relative z-10 p-3 sm:p-6">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end space-y-1 ml-2">
                    <motion.div whileHover={{ scale: 1.03, rotateZ: 3 }} style={{ transformStyle: "preserve-3d" }}>
                      <Badge
                        className={`${getStatusColor(benefit.status)} text-white text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 relative overflow-hidden`}
                      >
                        <motion.div
                        ...
                        />
                        <span className="relative z-10 text-xs font-medium">{benefit.status.toUpperCase()}</span>
                      </Badge>
                    </motion.div>
                    <motion.div
                      className={`text-sm sm:text-lg font-bold ${getTextColor(theme, "primary")}`}
                      animate={{
                ...
                <CardTitle className={`text-sm sm:text-lg mt-2 font-bold ${getTextColor(theme, "primary")}`}>
                  {benefit.title}
                </CardTitle>
                <CardDescription className={`text-xs sm:text-sm ${getTextColor(theme, "muted")} line-clamp-2`}>
                  {benefit.description}
                </CardDescription>
                ...
                <AnimatePresence>
                  {expandedCard === benefit.id && (
                    ...
                    >
                      <div className={`text-xs sm:text-sm ${getTextColor(theme, "secondary")}`}>
                        {benefit.detailedDescription}
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className={`${getTextColor(theme, "muted")}`}>
                          Total Savings: <span className="text-green-500 font-semibold">{benefit.savings}</span>
                        </span>
                        <span className={`${getTextColor(theme, "muted")}`}>Valid till: {benefit.validTill}</span>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
